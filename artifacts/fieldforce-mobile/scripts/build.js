...
// Keep the production static server (normally 8081) online while bundling.
const metroBuildPort = process.env.METRO_BUILD_PORT || '8082';
const metroBuildOrigin = `http://localhost:${metroBuildPort}`;
...
async function checkMetroHealth() {
  try {
    const response = await fetch(`${metroBuildOrigin}/status`, {
...
async function downloadBundle(platform, timestamp) {
  ...
  const url = new URL(`${metroBuildOrigin}/${bundlePath}.bundle`);
...
async function downloadManifest(platform) {
  ...
  try {
    ...
    const response = await fetch(`${metroBuildOrigin}/manifest`, {
...
function extractAssets(timestamp) {
  ...
  const extractFromBundle = (bundle, platform) => {
    for (const match of bundle.matchAll(assetPattern)) {
      ...
      const tempUrl = new URL(`${metroBuildOrigin}${originalPath}`);
...
async function downloadAssets(assets, timestamp) {
  ...
  const downloadPromises = assets.map(async (asset) => {
    const tempUrl = new URL(`${metroBuildOrigin}${asset.originalPath}`);
    const unstablePath = tempUrl.searchParams.get('unstable_path');
...
function updateBundleUrls(timestamp, baseUrl) {
  const updateForPlatform = (platform) => {
    ...
    bundle = bundle.replace(
      ...
      (_match, capturedPath) => {
        const tempUrl = new URL(`${metroBuildOrigin}${capturedPath}`);
        const unstablePath = tempUrl.searchParams.get('unstable_path');
